...
point_plot_throughput <- function()
{
  data_full <- read_data()
  # graph line graph
  ggplot(data = data_full, mapping = aes(x = trial_num, y = throughput_bytes_per_second, color = trial_name, shape = trial_name)) + geom_jitter() + facet_wrap(~ file_size_name)
}
...
graph_large_medium_real_time <- function()
{
  ...
  ggplot(data = large_medium, mapping = aes(x = trial_num, y = real_time_seconds, color = trial_name, shape = trial_name)) + geom_jitter() + facet_wrap(~ file_size_name)
}
...
graph_small_tiny_real_time <- function()
{
  ...
  ggplot(data = small_tiny, mapping = aes(x = trial_num, y = real_time_seconds, color = trial_name, shape = trial_name)) + geom_jitter() + facet_wrap(~ file_size_name)
}
